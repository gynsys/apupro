...
// AJUSTAR: Rutas a componentes y hooks existentes en tu proyecto
import Cost360SearchBar from '../components/Cost360SearchBar'; // placeholder
import { useCost360Search } from '../hooks/useCost360Search'; // placeholder
import { generatePartidasExcel } from '../../../../shared/lib/exportUtils';
import { deleteMasterItem } from '../../services/adminService';
...
const PartidasTab = ({ onlyCoded }) => {
  ...
  const handleExportToCsv = async () => {
    ...
    let exportItems = [];
    try {
      // AJUSTAR: Usa tu servicio existente para obtener todos los items
  ...
  const handleDelete = async (item) => {
    if (window.confirm("¿Estas seguro de eliminar esta partida maestra? Esto es irreversible y afectara a todos.")) {
      try {
        await deleteMasterItem(item.CodPar);
  ...
  return (
    <>
      <GlassCard className="rounded-2xl p-4 flex flex-col gap-3">
        {/* AJUSTAR: Integra tu Cost360SearchBar real aqui */}
        <div className="text-sm text-slate-500">
          Barra de busqueda (integra tu componente Cost360SearchBar aqui)
        </div>
