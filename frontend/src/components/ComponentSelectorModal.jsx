...
import { Search, X, Package, Wrench, Users, Check, Loader2, Database, ChevronDown, RefreshCw } from 'lucide-react';
...
const TYPE_CONFIG = {
  materials: {
    ...
    theme: {
      ...
      codeTag: 'text-blue-700 bg-blue-50 border-blue-100',
  ...
  equipments: {
    ...
    theme: {
      ...
      codeTag: 'text-indigo-700 bg-indigo-50 border-indigo-100',
  ...
  labors: {
    ...
    theme: {
      ...
      codeTag: 'text-emerald-700 bg-emerald-50 border-emerald-100',
    ...
    mapItem: (lab) => ({
      ...
      precio_unitario: Number(lab.Jornal || 0),
      cantidad: 1,
      unidad: 'Día'
    })
  }
...
export default function ComponentSelectorModal({
  ...
  const config = TYPE_CONFIG[type] || TYPE_CONFIG.materials;
  const { title, singular, plural, Icon, theme, placeholder, endpoint, mapItem } = config;
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      setSkip(0);
      fetchComponents(query, 0, false);
    }, 280);

    return () => clearTimeout(timer);
  }, [query]);

  const fetchComponents = async (searchStr, currentSkip, isAppend = false) => {
    if (isAppend) {
      setLoadingMore(true);
    } else {
      setLoading(true);
    ...
    try {
      ...
      const url = `${API_URL}${endpoint}?search=${encodeURIComponent(searchStr.trim())}&skip=${currentSkip}&limit=${LIMIT}${dbParam}`;
      const response = await fetch(url, {
        credentials: 'include'
      });

      if (!response.ok) {
        throw new Error(`Error al cargar ${plural}`);
      }
  ...
  const handleLoadMore = () => {
    ...
    setSkip(nextSkip);
    fetchComponents(query, nextSkip, true);
  };

  const handleSelectItem = (rawItem) => {
    const selectedData = mapItem(rawItem);
    if (onSelect) {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                {targetRow ? (
                  <span>
                    Selecciona un registro para asignar a la fila <strong className="text-slate-700 font-mono">[{targetRow.codigo || 'Nueva'}]</strong>
                  </span>
                ) : (
                  `Selecciona un elemento del catálogo de ${plural} para incluirlo en el análisis de precios`
                )}
              </p>
            </div>
          </div>
                    ...
                    {databases.map(db => (
                      <button
                        ...
                        className={`w-full text-left px-3 py-2 text-xs hover:bg-slate-50 transition-colors flex items-center gap-2 ${
                          activeDatabase.id === db.id ? 'bg-slate-100 font-bold text-slate-900' : 'text-slate-700'
                        }`}
                      >
                        <Database size={12} />
                        {db.name}
        ...
        <div className="p-4 bg-white border-b border-slate-100">
          <div className="relative">
            ...
            <input
              ...
              className={`w-full bg-slate-50 border border-slate-200 focus:bg-white focus:ring-2 rounded-xl pl-10 pr-10 py-2.5 text-xs text-slate-800 placeholder:text-slate-400 font-medium transition-all focus:outline-none ${theme.focusBorder}`}
            />
            {query && (
              <button
                type="button"
        ...
        <div className="flex-1 overflow-y-auto min-h-0 bg-slate-50/50">
          {loading && items.length === 0 ? (
            <div className="py-20 flex flex-col items-center justify-center text-slate-400 gap-2.5">
              ...
              <p className="text-xs font-semibold text-slate-600">Cargando {title.toLowerCase()}...</p>
            </div>
          ) : items.length === 0 ? (
            <div className="py-20 flex flex-col items-center justify-center text-slate-400 text-center px-4">
              ...
              {query && (
                <button
                  type="button"
                  onClick={() => setQuery('')}
                  className={`mt-3 text-xs font-bold hover:underline ${theme.link}`}
                >
              ...
              <thead className="bg-slate-100 text-slate-600 uppercase font-bold sticky top-0 border-b border-slate-200 z-10 shadow-sm text-[11px]">
                <tr>
                  ...
                  <th className="py-2.5 px-4">
                    {type === 'labors' ? 'Cargo / Descripción' : 'Descripción'}
                  </th>
                  {type === 'materials' && (
                    <th className="py-2.5 px-4 w-20 text-center">Unidad</th>
                  )}
                  {type === 'materials' && (
                  ...
                  {type === 'labors' && (
                    <>
                      <th className="py-2.5 px-4 w-28 text-right">Jornal ($)</th>
                      <th className="py-2.5 px-4 w-28 text-right">Bono ($)</th>
                      <th className="py-2.5 px-4 w-28 text-right">Total Día ($)</th>
                    </>
                  )}
              ...
              <tbody className="divide-y divide-slate-100 bg-white">
                {items.map((row, idx) => {
                  ...
                  return (
                    <tr
                      key={code || idx}
                      onDoubleClick={() => handleSelectItem(row)}
                      className={`${theme.rowHover} transition-colors cursor-pointer group`}
                    >
                      <td className="py-2.5 px-4 font-mono font-bold whitespace-nowrap">
                        <span className={`px-2 py-0.5 rounded border ${theme.codeTag}`}>
                          {code}
                        </span>
                      </td>
                      ...
                      {type === 'materials' && (
                        <td className="py-2.5 px-4 text-center font-semibold text-slate-600 whitespace-nowrap">
                          <span className="bg-slate-100 px-1.5 py-0.5 rounded text-[10px] uppercase">
                            {unit || 'UND'}
                          </span>
                        </td>
                      )}
                      {type === 'materials' && (
                        <td className="py-2.5 px-4 text-right font-bold text-slate-800 whitespace-nowrap">
                          {(price || 0).toLocaleString('es-VE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                        </td>
                      )}
                      {type === 'equipments' && (
                        <td className="py-2.5 px-4 text-right font-bold text-slate-800 whitespace-nowrap">
                      ...
                      {type === 'labors' && (
                        <>
                          <td className="py-2.5 px-4 text-right font-semibold text-slate-700 whitespace-nowrap">
                            {(row.Jornal || 0).toLocaleString('es-VE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                          </td>
                          <td className="py-2.5 px-4 text-right font-semibold text-slate-700 whitespace-nowrap">
                            {(row.Bono || 0).toLocaleString('es-VE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                          </td>
                          <td className="py-2.5 px-4 text-right font-bold text-slate-800 whitespace-nowrap">
                            {totalDia.toLocaleString('es-VE', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                          </td>
                        </>
                      )}
                      <td className="py-2.5 px-4 text-center whitespace-nowrap">
                        <button
                          ...
                          className={`inline-flex items-center justify-center gap-1 text-white font-bold text-[11px] px-3 py-1 rounded-lg shadow-sm transition-all active:scale-95 group-hover:shadow ${theme.button}`}
                        >
                          <Check size={12} />
                          Seleccionar
                        </button>
          ...
          {items.length > 0 && items.length < total && (
            <div className="p-4 text-center bg-white border-t border-slate-100">
              ...
              >
                {loadingMore ? (
                  <>
                    <RefreshCw size={13} className={`animate-spin ${theme.spinner}`} />
                    Cargando más {plural}...
                  </>
                ) : (
                  <>
                    <span>Cargar más {plural}</span>
