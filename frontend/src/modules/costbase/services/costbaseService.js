...
export const fetchItems = async (skip = 0, limit = 50, search = '', chapter = '', database_id = 'master', search_desc = true, search_insumos = false, covenin = '', only_coded = null) => {
  try {
    ...
    const hidden_categories = (final_only_coded === false) ? '' : (window.ARKO_SITE_CONFIG?.hiddenCategories?.join(',') || '');
...
export const cost360Service = costbaseService;
export default costbaseService;
