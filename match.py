...
df_staging_m = pd.read_csv('staging_materials.csv')
...
df_staging_e = pd.read_csv('staging_equipment.csv')
...
with open('matches_review.csv', 'w', newline='', encoding='utf-8') as f:
    writer = csv.writer(f)
...
with open('sync_prices.sql', 'w', encoding='utf-8') as f:
    f.write('\n'.join(sql_commands))
    
print("Done! Check matches_review.csv and sync_prices.sql")
